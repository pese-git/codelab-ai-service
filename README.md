...
Для работы с локальными моделями через Ollama используйте скрипт [`pull_model_docker.sh`](pull_model_docker.sh):

```bash
# Загрузить модель (например, qwen3:0.6b)
...
**Что делает скрипт:**
1. Проверяет, запущен ли контейнер Ollama
2. Если не запущен - автоматически запускает его через `docker compose up -d ollama`
3. Выполняет команду `ollama pull <модель>` внутри контейнера
4. Показывает список всех установленных моделей

**Доступные модели Ollama:**
- `qwen3:0.6b` - компактная модель (рекомендуется для начала)
- `llama3.2:1b` - легковесная модель от Meta
- `mistral:7b` - более мощная модель
- Полный список: https://ollama.com/library

**Примечание:** Модели сохраняются в Docker volume `ollama-data` и остаются доступными после перезапуска контейнеров.
...
# Остановка с удалением volumes (включая загруженные модели)
