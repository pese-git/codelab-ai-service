...
class BaseAgent(ABC):
    ...
    async def process(
        ...
        Note:
            stream_handler использует интерфейс IStreamHandler из Domain слоя,
            что позволяет Domain слою не зависеть от конкретной реализации
            из Application слоя. Это соблюдает Dependency Rule Clean Architecture.
        """
        pass
