"""
Маппер для преобразования между Session Entity и SessionModel.
...
class SessionMapper:
    """
    Маппер между доменной сущностью Session и моделью БД SessionModel.
    ...
    Пример:
        ...
        >>> # Entity -> Model
        >>> model = await mapper.to_model(session, db)
        >>> # Model -> Entity
    ...
    async def to_entity(
        ...
        Пример:
            >>> session_entity = await mapper.to_entity(session_model, db)
        """
        ...
        # Парсинг metadata сессии (если будет добавлено в модель)
        session_metadata = {}
    ...
    async def to_model(
        ...
        Пример:
            >>> session_model = await mapper.to_model(session_entity, db)
        """
        # Получить существующую модель или создать новую
        result = await db.execute(
            select(SessionModel).where(
                SessionModel.id == entity.id,
        ...
        if not model:
            ...
            logger.debug(f"Created new SessionModel for {entity.id}")
        else:
            ...
            logger.debug(f"Updated SessionModel for {entity.id}")
